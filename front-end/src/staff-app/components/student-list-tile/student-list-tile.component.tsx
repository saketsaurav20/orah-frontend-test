...
export const StudentListTile: React.FC<Props> = ({ isRollMode, student, stateList, setStateList, filter }) => {
  const [state, setState] = useState("all")
  return (<>
   {filter === "all" || filter === state ? <S.Container>
      ...
      {isRollMode && (
        <S.Roll>
          <RollStateSwitcher stateList={stateList} state={state} setState={setState}  filter={filter} setStateList={setStateList}/>
        </S.Roll>
      )}
    </S.Container>
    : null
